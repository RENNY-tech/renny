...
$dsn = 'mysql:dbname="データベース名";host=localhost';
    ...
    $stmt = $pdo->query($sql);
        ...
        $category = [];
        $stmt = $pdo->query("SELECT * FROM category_1 order by id asc");
        while ($row = $stmt->fetch()) { $category[$row['name']] = $row['name']; }
...
body {
    ...
    香水名  <input type="text" name="name" placeholder="香水名を入れてください！" ><br>
        ...
        香水のカテゴリー選択 <select name="category">
        <?php
            foreach ($category as $key=>$val) {
            printf('<option value="%s"%s>%s</option>', $key, ($key == $result[1]) ? " selected" : "", $val);
            }
        ?></select>
       <br>
       </div>
   ...
   写真のアップロード&emsp;<input type="file" name="upload_image" accept="image/*">     
